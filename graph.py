...
class Graph:
    ...
    def find_paths(self, from_v, to_v, max_stops=None, max_dist=None, exact=True):
        ...
        given then the method ends when a solution is found"""


        class State:
            """The live state of the algorithm's progress"""

            # we use state instead of self here because the super's self is needed
            def __init__(state):
                state.graph = self
        ...
        # trim the solution set if we need solutions with exact parameter matches
        if exact and max_stops:
            solutions = [sln for sln in solutions if sln.stops == max_stops]
        if exact and max_dist:
            solutions = [sln for sln in solutions if sum(link.magnitude for link in sln) == max_dist]
